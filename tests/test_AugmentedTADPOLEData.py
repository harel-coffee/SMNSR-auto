...
class TestAugmentedTADPOLEData(TestCase):
    ...
    TS_FILE = "fold_0.p"
    MERGED_TS_FILE = DUMP_PATH + "merged_all.p"
    MODALITY = "patient"
    SAVE_PATH = DUMP_PATH + "merged_saved"
    GOOGLE_DRIVE_URL = (
        "https://drive.google.com/uc?id=1diTUWzctbl5MfpgoKBuGa-hvIXVgJcx7"
    )
    _tadpole_data: TADPOLEData = TADPOLEData(modality_k=2, challenge_filter=True)
    ...
    def test_save(self):
        ...
        with bz2.BZ2File(self.SAVE_PATH, "rb") as file:
            ...
            self.assertTrue(isinstance(merged, tuple))
